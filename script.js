...
let box = 32;
let snake = []; //cria cobrinha como lista, já que ela vai ser uma série de coordenadas, que quando pintadas, criam os quadradinhos
snake[0] = {
    x: 8 * box,
    y: 8 * box
}
...
function iniciarJogo(){
    ...
    if(direction == "left") snakeX -= box; // para esquerda diminuiu
    if(direction == "up") snakeY -= box;
    if(direction == "down") snakeY += box;

    if(snakeX != food.x || snakeY != food.y){
        snake.pop(); //pop tira o último elemento da lista, ai muda o lugar da comida
    }else{
